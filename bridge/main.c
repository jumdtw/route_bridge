...
PARAM Param = {"enp0s31f6","wlp2s0",0};
            ...
            default:
                for(i=0;i<2;i++){
                    if(targets[i].revents&(POLLIN|POLLERR)){
                        if((size=read(Device[i].soc,buf,sizeos(buf)))<=0){
                            perror("read");
                        }else{
...
int DisableIpForward(){
    ...
    if((fp=fopen("/proc/sys/net/ipv4/ip_forward","w"))==NULL){
        DebugPrintf("cannt write /proc/sys/net/ipv4/ip_forward\n");
        return -1;
    }
    fput("0",fp);
    fclose(fp);
