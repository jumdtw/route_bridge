...
char *arp_ip2str(u_int8_t *ip,char *buf,socklen_t size){
    snprinf(buf,size,"%u.%u.%u.%u",ip[0],ip[1],ip[2],ip[3]);
...
int PrintArp(struct ether_arp *arp,FILE *fp){
    ...
    fprintf(fp,"arp_hl n=%u,",arp->arp_hln);
