...
//ioctlを使ってnicの情報を得ている
int GetDeviceInfo(char *device,u_char hwaddr[6],struct in_char *uaddr,struct in_addr *subnet,struct in_addr *mask){
    struct ifreq ifreq;
...
char *my_inet_ntoa_r(struct in_addr *add,char *buf,socklen_t size){
...
int SendArpRequestB(int  soc,in_addr_t target_ip,u_char target_mac[6],in_addr_t my_ip,u_char my_mac[6]){
    ...
    int total;
    u_char *p;
    u_char buf[siozeof(struct ether_header)+sizeof(struct ether_arp)];
    ...
    int i;
    arp.arp.arp_hrd = htons(ARPHED_ETHER);
    ...
    arp.eh.ether_shost[0] = target_mac[0];
    arp.eh.ether_shost[1] = target_mac[1];
    arp.eh.ether_shost[2] = target_mac[2];
    arp.eh.ether_shost[3] = target_mac[3];
    arp.eh.ether_shost[4] = target_mac[4];
    arp.eh.ether_shost[5] = target_mac[5];
