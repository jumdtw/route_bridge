...
PARAM Param = {"enp0s31f6","wlp2s0",1,"192.168.0.254"};
...
int SendIcmpTimeExceeded(int deviceNo,struct ether_header *eh,struct iphdr *iphdr,u_char *data,int size){
    ...
    rih->frag_off = 0;
    rih->ttl = 64;
    rih->protocol = IPROTO_ICMP;
    rih->check = 0;
    rih->saddr = Device[deviceNo]->add->s_addr;
    ...
    iphtr = data + sizeof(struct ether_header);
    ...
    DebugPrintf("write:SendIcmpTimeExceede:[%d] %dbytes\n",device,len);
    write(Device[deviceNo].soc,buf,len);

    return 0;
}

AnalyzePacket(int deviceNo,u_char *data,int size){
    u_char *ptr;
    int lest;
    ...
    if(lest<sizeof(struct ether_header)){
        DebugPrintf("[%d]:lest(%d)<sizeof(struct ether_header)\an",deviceNo,lest);
        return -1;
    }
    ...
    if(ntohs(eh->ehter_type)==ETHERTYPE_ARP){
        ...
        if(arp->arp_op == htons(ARPPOP_REQUEST)){
